...
const TABS = [
  { id: "profile" as const, label: "Tai khoan", icon: User },
  { id: "history" as const, label: "Lich su nghe", icon: Clock },
  { id: "follows" as const, label: "Dang theo doi", icon: BookOpen }
];
...
export function AccountPageClient({ tab: initialTab, account, history, follows }: Props) {
      ...
      <Card className="overflow-hidden">
        <CardContent className="p-0">
          <div className="flex flex-col items-center gap-6 p-6 sm:flex-row sm:p-8">
            ...
            <div className="flex-1 text-center sm:text-left">
              <div className="flex flex-wrap items-center justify-center gap-2 sm:justify-start">
                <h1 className="text-2xl font-black">{account.name ?? "Nguoi dung"}</h1>
                {account.isVip ? (
                  <Badge variant="accent" size="sm" className="inline-flex items-center gap-1"><Crown className="size-3" />VIP</Badge>
                ) : (
                  <Badge variant="secondary" size="sm">Mien phi</Badge>
                )}
                ...
                {isEditing ? (
                  <>
                    <Button size="sm" onClick={handleSave} disabled={editing}>
                      {editing ? <Loader2 className="mr-1 size-3.5 animate-spin" /> : <Check className="mr-1 size-3.5" />}Luu
                    </Button>
                    <Button size="sm" variant="secondary" onClick={cancelEdit} disabled={editing}>
                      <X className="mr-1 size-3.5" />Huy
                    </Button>
                  </>
                ) : (
                  <Button size="sm" variant="secondary" onClick={() => setIsEditing(true)}>
                    <Pencil className="mr-1 size-3.5" />Sua ho so
                  </Button>
                )}
                {!account.isVip ? (
                  <Button size="sm" asChild>
                    <Link href="/vip"><Crown className="mr-1 size-3.5" />Nang cap VIP</Link>
                  </Button>
                ) : null}
              </div>
              ...
              {isEditing ? (
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <div>
                    <label className="block text-xs font-semibold text-muted-foreground">Ten hien thi</label>
                    <input
                      type="text"
                      value={editName}
                      onChange={(e) => setEditName(e.target.value)}
                      className="mt-1 w-full rounded-lg border border-border/70 bg-background px-3 py-2 text-sm outline-none focus:border-ring focus:ring-1 focus:ring-ring"
          ...
          <div className="grid grid-cols-3 border-t border-border/60">
            <div className="p-4 text-center"><p className="text-lg font-black">{history.length}</p><p className="text-xs text-muted-foreground">Tap da nghe</p></div>
            <div className="border-x border-border/60 p-4 text-center"><p className="text-lg font-black">{follows.length}</p><p className="text-xs text-muted-foreground">Dang theo doi</p></div>
            <div className="p-4 text-center"><p className="text-lg font-black">{account.isVip ? "VIP" : "Free"}</p><p className="text-xs text-muted-foreground">Goi</p></div>
          </div>
        </CardContent>
      </Card>
      ...
      {activeTab === "profile" ? (
        <Card>
          ...
          <CardContent className="grid gap-2 text-sm">
            <div className="flex justify-between border-b border-border/40 pb-2"><span className="text-muted-foreground">Ten</span><span className="font-semibold">{account.name ?? "-"}</span></div>
            ...
            <div className="flex justify-between border-b border-border/40 pb-2">
              <span className="text-muted-foreground">Goi</span>
              <span className="font-semibold">{account.isVip ? "VIP" : "Mien phi"}</span>
            </div>
            <div className="flex justify-between border-b border-border/40 pb-2">
              <span className="text-muted-foreground">Vai tro</span>
              <span className="font-semibold">{account.role === "ADMIN" ? "Quan tri vien" : "Nguoi dung"}</span>
            </div>
            <div className="flex justify-between pb-2">
              <span className="text-muted-foreground">Tham gia tu</span>
            ...
            {history.length ? history.map((item) => (
              <div key={item.id} className="flex items-center gap-3 rounded-lg border border-border/60 p-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">Tap {item.episode.episodeNumber} - {item.episode.title}</p>
                  <p className="mt-0.5 text-xs text-muted-foreground">Da nghe {formatDuration(item.currentSeconds)}</p>
                </div>
                <Button asChild size="sm" variant="secondary" className="shrink-0">
                  <Link href={`/truyen/${item.episode.series.slug}/tap/${item.episode.episodeNumber}`}>Nghe tiep</Link>
                </Button>
              </div>
            )) : <p className="text-sm text-muted-foreground">Chua co lich su nghe.</p>}
          </CardContent>
        </Card>
      ) : null}
            ...
            {follows.length ? follows.map((follow) => (
              <Link key={follow.id} href={`/truyen/${follow.series.slug}`} className="flex items-center gap-3 rounded-lg border border-border/60 p-3 transition hover:border-accent/50 hover:bg-surface-3/50">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">{follow.series.episodeCount} tap</p>
                </div>
              </Link>
            )) : <p className="text-sm text-muted-foreground">Chua theo doi truyen nao.</p>}
          </CardContent>
        </Card>
      ) : null}
    </div>
